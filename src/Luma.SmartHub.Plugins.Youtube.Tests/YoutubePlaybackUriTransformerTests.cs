...
{
    public class YoutubePlaybackUriTransformerTests
    {
        [Fact]
        public void Should_Return_Proper_Audio_Uri()
        {
            ...
            var fixture = new YoutubePlaybackUriTransformerTestsFixture()
                .ReturnHtmlForUrl(exampleVideoUrl, "example-video-page.html");
            
            var result = fixture.Sut.Transform(new Uri(exampleVideoUrl));

            result.Should().Be(expectedAudioUri);
        }
        ...
        public void For_Unavailable_Video_Should_Throw_VideoNotAvailableException()
        {
            ...
            var act = new Action(() => fixture.Sut.Transform(new Uri(exampleVideoUrl)));

            Assert.Throws<VideoNotAvailableException>(act);
        }

        private class YoutubePlaybackUriTransformerTestsFixture
        {
            public YoutubePlaybackUriTransformer Sut { get; }

            public Mock<IWebClient> WebClient { get; }

            public YoutubePlaybackUriTransformerTestsFixture()
            {
                WebClient = new Mock<IWebClient>();
                Sut = new YoutubePlaybackUriTransformer(new DownloadUrlResolver(WebClient.Object));
            }

            public YoutubePlaybackUriTransformerTestsFixture ReturnHtmlForUrl(string url, string htmlPath)
            {
